...
Competition-host/community ideas (reported submission patterns and training
suggestions, not score evidence):
[`review_packets/competition_host_ideas_2026-08-15.md`](review_packets/competition_host_ideas_2026-08-15.md).
...
| H12 | A generic 2-D local recurrent grid can discover the learned compare/subtract-and-shift behavior without a hard-coded long-division schedule, because position-local tied cells have enough representational capacity and expose boundary states through neighboring refinement. | Public-E5 final-label-only matched test: 2-D local grid with learned phase state vs current 1-D tied tape, equal parameters and learned-cell applications, three seeds. Require >=95% exact on both 512-row T=1 profiles for every seed before recurrence or Hard. |
| H13 | Maintaining latent prefix and prefix-square registers during a learned MSB-to-LSD scan avoids the raw-square magnitude/quotient bottleneck and makes T=1 learnable from final labels. | Parameter/compute-matched public-E5 final-label-only arm against H12 and the 1-D tape; the cell receives generic register/context features only, with no coded arithmetic update. Same 95%/95% promotion gate. |
| H14 | The directly supervised 11-bit Neural GPU squarer contains a reusable square representation that can improve final-label modular learning, rather than merely fitting the standalone task. | Four-arm controlled transfer: A frozen pretrained squarer + learned reducer; B trainable pretrained squarer + learned reducer; C random squarer + learned reducer; D fully entangled end-to-end model. Freeze splits, reducer, optimizer, compute, and seeds. Compare raw-square retention, final modular exactness on seen and unseen N, and gradient flow into the square phase. A>B/C supports reusable features; B>A supports useful adaptation; C matching B refutes pretraining value; D winning supports entangled credit assignment. Research-only until a legal final-label arm passes the promotion gate. |
| H15 | Varying-N failure is caused primarily by modulus information being stored in mutable state and overwritten during reduction. | Reinject the same learned N features at every reducer microstep; otherwise freeze source, optimizer, data, seed, width, and depth. Require improvement on E3 and E5 without reducing E1 by more than 3 points. |
| H16 | Allowing the square phase to see N encourages an entangled fixed-N shortcut that cannot transfer across moduli. | Replace square-phase N row with a learned null while retaining N in the reducer; matched E1/E3/E5 comparison and direct raw-square probe. |
| H17 | Tuned Muon's high early learning rate is useful for fast circuit formation, but a late decay will reduce the observed recurrent-update excursions and improve the validation-selected endpoint. | At width 128, freeze the successful Muon 0.006 recipe through step 5,000, then compare constant LR against one preregistered cosine decay to 0.001 through step 10,000. Require higher validation and both unseen-N audits, not merely a smoother curve. |
| H18 | Nine percent recurrent dropout is slowing final-label fused train fit more than it helps this varying-N regime; scheduling dropout from zero early to 0.09 after circuit formation may preserve generalization while reducing steps to first signal. | Freeze width-128 tuned Muon and compare constant 0.09 dropout against a preregistered 0→0.09 schedule. Select on validation; open audits once. Track steps to 5%, 10%, and 15% train exact as well as endpoints. |
