...
| H12 | Once a tied learned reduction transition has full intermediate-state support and stable rollout, a learned stop/canonicality head can replace evaluator-supplied q without destroying q≥4 exactness. | Separate Phase-2 card: retain the full-trace transition cell and add only a learned stop head; test whether it halts at the canonical state without access to q. |
