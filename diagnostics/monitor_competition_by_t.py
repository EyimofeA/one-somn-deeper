"""Local training monitor with checkpointing and exact seen/OOD-N T profiles.
...
COMPETITION_ROOT = Path(__file__).resolve().parents[1] / "competition"
...
def main() -> None:
    ...
    seen_t_splits = _depth_split_names(dataloaders)
    ood_n_t_splits = _depth_split_names(dataloaders, "depth_ood_n_t_")
    evaluation_splits = (*scoring_splits, *seen_t_splits, *ood_n_t_splits)

    print(f"evaluation splits: {evaluation_splits}", flush=True)
            ...
            for split_name in evaluation_splits:
                try:
                    metrics = _evaluate(
                        ...
                        manifest,
                        device,
                        deadline=time.monotonic() + args.eval_budget_seconds,
                ...
                log(
                    {
                        ...
                        "split": split_name,
                        "profile": (
                            "ood_n_t" if split_name.startswith("depth_ood_n_t_")
                            else "seen_t" if split_name.startswith("depth_t_")
                            else "aggregate"
                        ),
                        "step": step,
                        "elapsed_seconds": elapsed,
