"""Output place-value scaffold on the place-value UT loop — claude code.

One-change ablation vs `claude_pv_k4_ut` (C1).

Motivation (claude code): the benchmark reads the answer off the LAST len(answer)
positions of the input. `collate_squaring_mod` sets

    target_positions[row, :target_len] = arange(input_len - target_len, input_len)

so the final real position of every sequence is the answer's units digit, and a
position's distance from the sequence end IS the place value of the digit it must
emit there — invariant to how many digits N, X and T happen to have.

C1 gave the block place value for the *inputs* (N/X/T spans) but nothing for the
*output*. To know "I am the hundreds digit of y" the model had to infer its own
offset from the end via absolute position, which shifts whenever operand lengths
shift. That predicts the observed split pattern: e1 (fixed N=323) has near-stable
lengths and scores 5-7%, while e3/e5 (10-11 bit N, so variable lengths) sit at
floor <1.5%. This supplies the missing signal directly.

Single change vs C1: an `answer_place` embedding indexed by
(true_length - 1 - position), derived from the attention mask (or PAD id) only —
no dataset access, no label access, and no `target_positions` (which `forward`
never receives anyway). Everything else — d=32, 4 heads, shared Block x K=4, UT
depth embeddings, field/place embeddings, AdamW warmup+cosine, batch 256 — is
held identical to C1. Added state: 64 x 32 = 2,048 elements.
"""
...
def _field_and_place(input_ids: Tensor) -> tuple[Tensor, Tensor]:
    ...
    field_id in {0,1,2,3}: 0 = pre-N padding, 1 = N span, 2 = X span, 3 = T span.
    place_id in {0..MAX_PLACES-1}: 0 for non-digits; for a digit it is
    1 + (distance from the least-significant digit of its span), so the units
    digit of every operand shares place_id == 1 regardless of operand length.
    """
...
def _answer_place(input_ids: Tensor, attention_mask: Tensor | None) -> Tensor:
    """Distance from the end of the real (unpadded) sequence. claude code.

    Index 0 is the final real position — always the answer's units digit — so the
    output place value is invariant to operand lengths. Padding clamps to 0 and is
    never attended to anyway.
    """
    if attention_mask is not None and attention_mask.dim() == 2:
        real = attention_mask.to(torch.bool)
    else:
        real = input_ids != 0
    lengths = real.sum(dim=1, keepdim=True)
    positions = torch.arange(input_ids.shape[1], device=input_ids.device)[None, :]
    return (lengths - 1 - positions).clamp_(0, MAX_PLACES - 1).long()
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.answer_place_embedding = nn.Embedding(MAX_PLACES, D_MODEL)
        ...
        nn.init.zeros_(self.answer_place_embedding.weight)
    ...
    def forward(
        ...
        answer_place = _answer_place(input_ids, attention_mask)
        x = (
            self.token_embedding(input_ids)
            + self.position_embedding(positions)
            + self.segment_embedding(field)
            + self.place_embedding(place)
            + self.answer_place_embedding(answer_place)
        )
