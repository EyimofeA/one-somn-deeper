...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    ...
    scaled_ce = (token_ce / smooth_max_temperature).masked_fill(
        ~batch.valid_mask, -torch.inf
    )
    row_ce = smooth_max_temperature * (
        torch.logsumexp(scaled_ce, dim=1) - torch.log(valid_count)
    )
