"""Train K=2 / eval K=4 with ZERO-INIT depth embeddings — claude code.

One change vs `depth_d32_k2_ut_evalk4` (current e1 best, 6.83%): zero-init
`depth_embedding`. That card trains only slots k=0,1 but evaluates with 4
loops, so loops 3-4 add default N(0,1) vectors that never saw a gradient.
Zero-init makes those loops clean extra block applications instead.

Original docstring follows.

Depth-looped Transformer with Universal-Transformer-style depth embeddings.

d_model=32, shared Pre-LN Block. Train with K=2 loops (cheap); eval with K=4
(extra depth). Each loop step adds a learned depth embedding (Dehghani et al.,
2018 — Universal Transformers) so the shared block can distinguish timestep k.
This is fixed-depth recurrence, not Adaptive Computation Time (ACT).
"""
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        # Slots k=2,3 never get a gradient (train_loops=2) yet are used at
        # eval (eval_loops=4). Default N(0,1) init would inject an untrained
        # unit-variance vector into a d=32 stream on those loops; zero-init
        # makes them pure extra block applications. claude code
