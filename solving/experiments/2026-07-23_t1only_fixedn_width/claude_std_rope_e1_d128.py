...
Width variant of `2026-07-22_claude_std_rope_e1`: d_model 32 -> 128 (4x), everything
else unchanged (heads=4, depth=4, RoPE, plain token embedding, GELU MLP, AdamW wd=0.1,
wall-clock schedule). Isolates whether rung 1's near-floor result
(`2026-07-23_t1only_fixedn_wd01`, N=1073: 0.00% test EM) is a representational
ceiling or a d=32 capacity artifact.

Same d_model=128, same heads=4, same depth=4 across this width sweep (see also
the d=256 and _b115 (N=323) siblings in this directory).

Input is otherwise the plainest possible: `token_embedding(input_ids)` only.

Everything else (AdamW, batch 256) matches the d=32 anchor. Scheduler is the
