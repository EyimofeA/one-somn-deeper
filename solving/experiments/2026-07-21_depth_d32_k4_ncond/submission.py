"""Depth-looped Transformer with N-conditioning — d_model=32, K=4, FiLM from N span."""
...
# competition/data/squaring_mod.py TOKEN_IDS
TOKEN_N = 2
TOKEN_X = 3
...
def _pool_n_span(embeddings: Tensor, input_ids: Tensor) -> Tensor:
    """Mean-pool embeddings strictly between N and X markers; zeros if invalid."""
    batch, seq_len, d_model = embeddings.shape
    device = embeddings.device
    positions = torch.arange(seq_len, device=device)
    is_n = input_ids == TOKEN_N
    is_x = input_ids == TOKEN_X
    sentinel = seq_len + 1
    n_first = torch.where(is_n, positions, sentinel).min(dim=1).values
    x_first = torch.where(is_x, positions, sentinel).min(dim=1).values
    valid = (n_first < sentinel) & (x_first < sentinel) & (x_first > n_first + 1)
    pos = positions.unsqueeze(0)
    between = (pos > n_first.unsqueeze(1)) & (pos < x_first.unsqueeze(1))
    between = between & valid.unsqueeze(1)
    mask = between.unsqueeze(-1).to(embeddings.dtype)
...
class Model(nn.Module):
    ...
    def forward(
        ...
        n = self.n_proj(_pool_n_span(x, input_ids))
        for _ in range(self.num_loops):
            gamma, beta = self.film(n).chunk(2, dim=-1)
