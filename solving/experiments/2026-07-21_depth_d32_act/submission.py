"""Depth-looped Transformer with ACT-style soft halting — d_model=32, K_MAX=8."""
...
def masked_mean(x: Tensor, attention_mask: Tensor | None) -> Tensor:
    """Mean-pool over sequence; mask shape [B, L] with 1=keep."""
    if attention_mask is None:
        return x.mean(dim=1)
    batch, length, _ = x.shape
    if attention_mask.shape != (batch, length):
        raise ValueError("invalid attention_mask shape for pooling")
...
class Model(nn.Module):
    num_loops = 8  # K_MAX — max block applications per forward
    ...
    def forward(
        ...
        batch = x.shape[0]
        h = x
        remainder = torch.ones(batch, device=x.device, dtype=x.dtype)
        out = torch.zeros_like(h)
        for k in range(K_MAX):
            h = self.block(h, attention_mask)
            pooled = masked_mean(h, attention_mask)
            p = torch.sigmoid(self.halt_proj(pooled)).squeeze(-1)
            if k == K_MAX - 1:
                p = torch.ones_like(p)
            w = remainder * p
            out = out + w[:, None, None] * h
            remainder = remainder * (1 - p)
        return self.head(self.final_norm(out)), None
