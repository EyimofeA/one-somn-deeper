"""Canonical recurrence with one learned local digit-mixing residual.
...
used by the model or loss. A tied ConvGLU residual lets adjacent LSD-first
state digits exchange local information before every recurrent output.
"""
...
class LocalDigitMixer(nn.Module):
    """Translation-shared neighbor update; it contains no arithmetic rule."""

    def __init__(self) -> None:
        super().__init__()
        self.norm = RMSNorm(D_MODEL)
        self.neighbor = nn.Conv1d(
            D_MODEL, D_MODEL, kernel_size=3, padding=1, groups=D_MODEL
        )
        self.glu = nn.Linear(D_MODEL, 2 * D_MODEL)
        self.scale = nn.Parameter(torch.tensor(LOCAL_RESIDUAL_INIT))

    def forward(self, x: Tensor) -> Tensor:
        local = self.neighbor(self.norm(x).transpose(1, 2)).transpose(1, 2)
        gate, value = self.glu(local).chunk(2, dim=-1)
        return x + self.scale * torch.sigmoid(gate) * torch.tanh(value)
...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.step = StepBlock()
        self.local_digit_mixer = LocalDigitMixer()
        ...
        for module in self.modules():
            if isinstance(module, nn.Embedding):
                ...
                nn.init.normal_(
                    module.weight,
                    std=INIT_SCALE
                    * math.prod(module.weight.shape[1:]) ** -0.5,
                )
    ...
    def forward(
        ...
        for step_index in range(maximum_t):
            ...
            hidden = self.local_digit_mixer(hidden)
...
class DiagnosticSchedule:
    """Read-only local-branch telemetry after each ordinary optimizer step."""

    def __init__(self, base: WallClockSchedule, model: Model) -> None:
        self.base = base
        self.model = model
        self.steps = 0
    ...
    def step(self) -> None:
        ...
        if self.steps == 1 or self.steps % 100 == 0:
            mixer = self.model.local_digit_mixer
            gradient = mixer.scale.grad
            print(
                "LOCAL_DIAGNOSTIC "
                f"step={self.steps} scale={float(mixer.scale.detach()):.8f} "
                f"scale_grad={float(gradient.detach()) if gradient is not None else float('nan'):.8f} "
                f"neighbor_norm={float(mixer.neighbor.weight.detach().norm()):.6f} "
                f"glu_norm={float(mixer.glu.weight.detach().norm()):.6f}",
                flush=True,
            )
...
def build_optimizer(model: nn.Module, spec: OptimizerSpec) -> OptimizerBundle:
    ...
    base_schedule = WallClockSchedule(optimizer, spec.training_time_seconds)
    return OptimizerBundle(optimizer, DiagnosticSchedule(base_schedule, model))
