...
class Model(nn.Module):
    ...
    def forward(
        ...
        self.auxiliary = {"t_value": t_value, "n_length": n_valid.sum(1)}
        return logits, self.auxiliary


def token_training_loss(batch: TokenLossBatch) -> Tensor:
    group_temperature = 0.25
    ...
    if (
        ...
        or "t_value" not in auxiliary
        or "n_length" not in auxiliary
    ):
        return row_ce.mean()
    ...
    group_index = (auxiliary["n_length"] - 1).clamp(0, MAX_DIGITS - 1)
    membership = F.one_hot(group_index, MAX_DIGITS).to(row_ce.dtype)
    group_mass = (membership * weights[:, None]).sum(0)
    group_loss = (membership * (row_ce * weights)[:, None]).sum(0) / (
        group_mass.clamp_min(1.0)
    )
    present_loss = group_loss[group_mass > 0]
    return group_temperature * (
        torch.logsumexp(present_loss / group_temperature, dim=0)
        - torch.log(present_loss.new_tensor(float(present_loss.numel())))
    )
