"""CMC-Hard: recurrent tape with causally consumed directional messages.
...
One causal change from the runnable GPT-5 Pro tape scaffold: learned left/right
messages are explicitly routed into the next content-state update.
"""
...
class TapeTransition(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.message_write = nn.Linear(WORK_CHANNELS, 2 * MESSAGE_CHANNELS)
        self.message_read = nn.Linear(2 * MESSAGE_CHANNELS, WORK_CHANNELS)
    ...
    def forward(self, tape: Tensor, ctrl: Tensor) -> Tensor:
        ...
        send_left, send_right = self.message_write(self.norm(h)).chunk(2, dim=-1)
        from_left = F.pad(send_right[:, :-1], (0, 0, 1, 0))
        from_right = F.pad(send_left[:, 1:], (0, 0, 0, 1))
        received = self.message_read(torch.cat((from_left, from_right), dim=-1))
        g, u = self.gate(self.norm(h + attn + received)).chunk(2, dim=-1)
