...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.place_embedding = nn.Embedding(MAX_DIGITS, FEATURE_DIM)
        self.n_local = nn.Sequential(
            nn.Linear(FEATURE_DIM, STATE_DIM),
            nn.LayerNorm(STATE_DIM),
            nn.GELU(),
        )
        self.x_local = nn.Sequential(
            nn.Linear(FEATURE_DIM, STATE_DIM),
            nn.LayerNorm(STATE_DIM),
            nn.GELU(),
        )
        self.initialize_state = nn.Sequential(
            nn.Linear(2 * STATE_DIM, STATE_DIM),
            nn.LayerNorm(STATE_DIM),
            nn.GELU(),
        )
        self.local_mix = nn.Linear(3 * STATE_DIM, STATE_DIM)
        self.update = nn.GRUCell(2 * STATE_DIM, STATE_DIM)
        self.decoder = nn.Sequential(
            nn.Linear(STATE_DIM + FEATURE_DIM, STATE_DIM),
            nn.GELU(),
            nn.Linear(STATE_DIM, spec.vocab_size),
        )
    ...
    def forward(
        ...
        n_tokens, x_tokens, _n_valid, t_value = _parse_prompt(
        ...
        n_features = self.token_embedding(n_tokens) + place_features
        x_features = self.token_embedding(x_tokens) + place_features
        n_state = self.n_local(n_features)
        state = self.initialize_state(
            torch.cat([n_state, self.x_local(x_features)], dim=-1)
        )
        ...
        for step_index in range(maximum_t):
            ...
            local = self.local_mix(torch.cat([left, state, right], dim=-1))
            proposed_state = self.update(
                torch.cat([local, n_state], dim=-1).flatten(0, 1),
                state.flatten(0, 1),
            ).view_as(state)
            proposed_logits = self.decoder(
                torch.cat([proposed_state, place_features.expand(batch, -1, -1)],
                          dim=-1)
            )
