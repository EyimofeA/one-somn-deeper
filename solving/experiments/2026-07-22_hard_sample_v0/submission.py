"""Hard-oriented sample — recurrence-agnostic tied loops + place/field.

Not a claimed Hard solution. See ASSUMPTIONS.md beside this file.
"""
...
NUM_HEADS = 4
NUM_LOOPS = 8
MARKER_N, MARKER_X, MARKER_T = 2, 3, 4
DIGIT_LO, DIGIT_HI = 7, 16
MAX_PLACES = 64
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        # Smaller init scale on token emb (Omnigrok-style knob; see ASSUMPTIONS).
        with torch.no_grad():
    ...
    def forward(
        ...
        for k in range(self.num_loops):
            # Input injection each loop (re-add base).
            depth = self.depth_embedding(
                torch.full((), k, device=input_ids.device, dtype=torch.long)
            )
            x = self.block(x + base + depth, attention_mask)
...
def _build_scheduler(
    ...
    # Clamped cosine: no restart after T_max (Medium/Hard wall-clock).
