...
class Model(nn.Module):
    ...
    def forward(
        ...
        # ABLATION (fable_max_nohardst): isolates whether the discrete
        # hard_st=prog>0.55 jump (an instant switch in the forward VALUE of
        # p, not just its gradient path) is what caused fable_max's e5
        # collapse (loss 0.29->2.18 at progress~0.69), vs. theta's anneal.
        # theta is UNCHANGED from the original; hard_st is forced permanently
        # False, so p stays pure soft softmax the entire run — same cheap
        # branch the original used pre-collapse, no extra compute added (so
        # this doesn't reintroduce fable_max_smooth's step-count confound).
        if self.training:
