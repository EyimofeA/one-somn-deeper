"""Place-value UT loop with absolute position REMOVED — claude code.

One-change ablation vs `claude_pv_k4_ut` (C1): delete `position_embedding`.

Motivation (claude code): (field, place) is already a COMPLETE positional code for
this task's inputs. Within a span, `place` (distance from that operand's units
digit) uniquely identifies a digit; across spans, `field` distinguishes N from X
from T; each marker is (field=f, place=0) and is therefore unique; padding is
(0, 0). So no two distinct roles collide, and absolute position adds no
information C1 does not already have.

It does, however, add a liability. Absolute position is the key that lets the
model memorise "the digit at index 7" instead of computing "the units digit of
X". That crutch works only while sequence lengths are stable, which is exactly
the regime where the current cards score well (e1, fixed N=323) and collapses
where they do not (e3/e5, 10-11 bit N, variable lengths). The measured splits
back this: C1 scores 3.3% test / 9.0% ood on e1 but 1.3/1.4 on e3.

Removing absolute position forces the block to route by place value alone. It is
also free — strictly fewer parameters and identical wall-clock.

Everything else (d=32, 4 heads, shared Block x K=4, UT depth embeddings, field +
place embeddings, AdamW warmup+cosine, batch 256) is held identical to C1.
"""
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        # No position_embedding — (field, place) is a complete positional code.
        ...
        # Structural embeddings now CARRY position, so they must not start at
        # zero the way they did in C1 (there they were a delta on top of an
        # existing absolute-position signal). claude code
        nn.init.normal_(self.segment_embedding.weight, std=0.02)
        nn.init.normal_(self.place_embedding.weight, std=0.02)
