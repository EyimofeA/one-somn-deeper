...
A11. v2 (post fable_max evidence): quantization hardening is gated on the
    register digits' confidence EMA, never on wall clock, and training alpha is
    capped at 0.90. Rationale: both monitored fable_max trajectories collapsed
    irreversibly to the digit-marginal floor (test loss ~2.17 = position-
    conditioned digit marginals, vs ln10=2.303 uniform) exactly when the
    wallclock schedule drove alpha to 1.0 (progress ~0.7), closing the only
    information path around an imperfect digit_head. Mixer is SwiGLU-gated:
    squaring is built from digit-pair products and a GELU MLP has no
    multiplicative primitive. Eval remains fully hard (alpha=1, argmax).
=============================================================================
"""
...
CARRY_STATES = 16       # learned discrete carry/borrow alphabet
FILM_FEATS = 8
...
class FilmBlock(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.gate = nn.Linear(D_MODEL, MLP_MULT * D_MODEL)
...
class DigitMicroScan(nn.Module):
    """One shared register-digit transition with a discrete carry channel."""

    def __init__(self) -> None:
        super().__init__()
        self.context_norm = RMSNorm(D_MODEL)
        self.carry_embedding = nn.Embedding(CARRY_STATES, D_MODEL)
        self.start_logits = nn.Parameter(torch.zeros(CARRY_STATES))
        self.up = nn.Linear(D_MODEL, MLP_MULT * D_MODEL)
        self.gate = nn.Linear(D_MODEL, MLP_MULT * D_MODEL)
        self.down = nn.Linear(MLP_MULT * D_MODEL, D_MODEL)
        self.carry_head = nn.Linear(MLP_MULT * D_MODEL, CARRY_STATES)

    @staticmethod
    def _discretize(logits: Tensor, training: bool) -> Tensor:
    ...
    def initial_carry(
        ...
        logits = self.start_logits.to(device=device, dtype=dtype)
        logits = logits.unsqueeze(0).expand(batch, -1)
        return self._discretize(logits, self.training)

    def forward(self, context: Tensor, carry: Tensor) -> tuple[Tensor, Tensor]:
        carry_value = carry @ self.carry_embedding.weight.to(carry.dtype)
        x = self.context_norm(context) + carry_value.to(context.dtype)
        hidden = F.silu(self.gate(x)) * self.up(x)
        digit_state = context + self.down(hidden)
        carry_logits = self.carry_head(hidden)
        next_carry = self._discretize(carry_logits, self.training)
        return digit_state, next_carry
...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.step_operator = DigitMicroScan()
        ...
        self.ema_conf = 0.10  # plain float; register-confidence EMA (A11)
    ...
    def forward(
        ...
        field, place, t_value, reg, row_len = self._parse(input_ids, mask)
        ...
        reg_place = row_len.unsqueeze(1) - 1 - pos
        max_reg_places = (
            int(reg.long().sum(dim=1).max().item()) if batch else 0
        )
        ...
        # quantization schedule: CAPABILITY-gated (A11). Hardening follows the
        # register digits' own confidence (EMA of max-prob); alpha is capped at
        # 0.90 in training so the residual (1-alpha)*h channel never closes.
        # Closing it (alpha->1.0 at wallclock progress ~0.7) caused the
        # unrecoverable collapse to the digit-marginal floor in both monitored
        # fable_max runs (cliffs at 38-41s/55.2s and 325.7s/460s horizons).
        if self.training:
            ramp = min(1.0, max(0.0, (float(self.ema_conf) - 0.45) / 0.45))
            theta = max(0.2, 1.0 - 0.8 * ramp)
            alpha = 0.25 + 0.65 * ramp
            hard_st = ramp > 0.85
        else:
        ...
        for t in range(max_loops):
            row_gate = t < loops                                     # (B,)
            gate = row_gate.to(h.dtype)[:, None, None]               # (B,1,1)
            feats = loop_feats_cache[:, t]
            z = self.block_b(self.block_a(h, attn_mask, feats), attn_mask, feats)

            # Context is computed in parallel, then frozen. Register writes
            # run from each row's real end (place zero) toward its MSD; the
            # discrete carry is the scan's only cross-place communication.
            carry = self.step_operator.initial_carry(
                batch, device=device, dtype=h.dtype
            )
            for digit_place in range(max_reg_places):
                slot = reg & (reg_place == digit_place)
                active = row_gate & slot.any(dim=1)
                context = (z * slot.unsqueeze(-1).to(z.dtype)).sum(dim=1)
                digit_state, next_carry = self.step_operator(context, carry)
                write = slot.unsqueeze(-1) & active[:, None, None]
                h = torch.where(write, digit_state[:, None, :], h)
                active_f = active[:, None].to(carry.dtype)
                carry = carry + active_f * (next_carry - carry)
            ...
            if self.training:
                ...
                conf_sum = conf_sum + (p_e.max(dim=-1).values * sel).sum()
        ...
        if self.training:
            with torch.no_grad():
                batch_conf = float(
                    (conf_sum / entropy_count.clamp(min=1.0)).item()
                )
            self.ema_conf = 0.98 * self.ema_conf + 0.02 * batch_conf
            ramp_now = min(1.0, max(0.0, (self.ema_conf - 0.45) / 0.45))
            lam = 0.02 + 0.10 * ramp_now
