...
ORIGINAL_OOD_X_COUNT = 100
PRIMING_X_COUNT = 3
MULTIPLIERS = tuple(range(10))
ORIGINAL_TRAIN_SIZE = sum(TRAIN_X_COUNTS.values()) * len(MULTIPLIERS)
PRIMING_SIZE = PRIMING_X_COUNT * len(MULTIPLIERS)
TRAIN_SIZE = ORIGINAL_TRAIN_SIZE + PRIMING_SIZE
TEST_SIZE = sum(TEST_X_COUNTS.values()) * len(MULTIPLIERS)
OOD_SIZE = (ORIGINAL_OOD_X_COUNT - PRIMING_X_COUNT) * len(MULTIPLIERS)
DEFAULT_OUTPUT_DIR = Path("data/generated/gate1_length4_priming")
...
def select_original_x_splits() -> dict[str, list[int]]:
    ...
    return {
        "train": train_x,
        "test": test_x,
        "ood": ood_candidates[:ORIGINAL_OOD_X_COUNT],
    }


def select_x_splits() -> dict[str, list[int]]:
    original = select_original_x_splits()
    priming_x = original["ood"][:PRIMING_X_COUNT]
    return {
        "train": [*original["train"], *priming_x],
        "test": original["test"],
        "ood": original["ood"][PRIMING_X_COUNT:],
    }


def build_records_for_splits(
    x_splits: dict[str, list[int]],
) -> list[dict[str, Any]]:
...
def build_records() -> list[dict[str, Any]]:
    return build_records_for_splits(select_x_splits())
...
def self_test(records: list[dict[str, Any]]) -> None:
    ...
    assert len(x_sets["train"]) == (
        sum(TRAIN_X_COUNTS.values()) + PRIMING_X_COUNT
    )
    assert len(x_sets["test"]) == sum(TEST_X_COUNTS.values())
    assert len(x_sets["ood"]) == ORIGINAL_OOD_X_COUNT - PRIMING_X_COUNT
    ...
    assert sum(len(str(x)) == 4 for x in x_sets["train"]) == PRIMING_X_COUNT
    assert all(len(str(x)) == 4 for x in x_sets["ood"])

    original_splits = select_original_x_splits()
    priming_x = original_splits["ood"][:PRIMING_X_COUNT]
    assert x_sets["train"] == set(original_splits["train"]) | set(priming_x)
    assert x_sets["test"] == set(original_splits["test"])
    assert x_sets["ood"] == set(original_splits["ood"][PRIMING_X_COUNT:])
    original_records = build_records_for_splits(original_splits)
    original_by_split = {
        split: [
            record for record in original_records if record["split"] == split
        ]
        for split in ("train", "test")
    }
    assert by_split["train"][:ORIGINAL_TRAIN_SIZE] == (
        original_by_split["train"]
    )
    assert by_split["test"] == original_by_split["test"]
    ...
    for x in priming_x:
        ...
        assert all(record["x"] != x for record in by_split["ood"])
...
def dataset_config(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "generator_config": {
            ...
            "ood_length": 4,
            "original_ood_x_count": ORIGINAL_OOD_X_COUNT,
            "priming_examples": PRIMING_SIZE,
            "priming_fraction_of_original_train": (
                PRIMING_SIZE / ORIGINAL_TRAIN_SIZE
            ),
            "priming_fraction_of_resulting_train": PRIMING_SIZE / TRAIN_SIZE,
            "priming_x": select_original_x_splits()["ood"][:PRIMING_X_COUNT],
            "remaining_ood_x_count": (
                ORIGINAL_OOD_X_COUNT - PRIMING_X_COUNT
            ),
            "seed": SEED,
