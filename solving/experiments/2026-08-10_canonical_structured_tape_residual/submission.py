...
class StructuredTape(nn.Module):
    """Generic LSD-aligned local workspace; it encodes no arithmetic rule."""

    def __init__(self) -> None:
        super().__init__()
        self.state_to_tape = nn.Linear(D_MODEL, TAPE_DIM)
        self.n_to_tape = nn.Linear(D_MODEL, TAPE_DIM)
        self.local_mix = nn.Linear(3 * TAPE_DIM, TAPE_DIM)
        self.update = nn.GRUCell(2 * TAPE_DIM, TAPE_DIM)
        self.to_model = nn.Linear(TAPE_DIM, D_MODEL)

    def forward(self, mutable: Tensor, n_context: Tensor) -> Tensor:
        state = F.gelu(self.state_to_tape(mutable))
        modulus = F.gelu(self.n_to_tape(n_context))
        zero = torch.zeros_like(state[:, :1])
        left = torch.cat([zero, state[:, :-1]], dim=1)
        right = torch.cat([state[:, 1:], zero], dim=1)
        local = F.gelu(self.local_mix(torch.cat([left, state, right], dim=-1)))
        updated = self.update(
            torch.cat([local, modulus], dim=-1).flatten(0, 1),
            state.flatten(0, 1),
        ).view_as(state)
        return mutable + self.to_model(updated)
...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.structured_tape = StructuredTape()
        self.step = StepBlock()
    ...
    def forward(
        ...
        for step_index in range(maximum_t):
            ...
            mutable = self.structured_tape(mutable, n_context)
