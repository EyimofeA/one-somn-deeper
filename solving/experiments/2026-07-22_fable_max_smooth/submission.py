...
class Model(nn.Module):
    ...
    def forward(
        ...
        # FIX (fable_max_smooth): the original used a discrete `hard_st = prog
        # > 0.55` switch between two different forward formulas for `p` (pure
        # soft softmax below the threshold, hard straight-through above it) —
        # an instantaneous jump in the actual computed q_state, not just a
        # gradient-path change. fable_max's e5 run collapsed catastrophically
        # (loss 0.29 -> 2.18, 80% -> 0% train accuracy) at progress~0.69, just
        # past that threshold. Replaced with `mix`, a continuous 0->1
        # interpolation of the FORWARD VALUE between soft and
        # hard-with-straight-through-gradient — same schedule variable
        # (prog/0.7) already driving theta/alpha, so no new discontinuity is
        # introduced, and no step introduces a new formula.
        if self.training:
            prog = float(self.progress)
            theta = max(0.15, 1.0 - 0.85 * min(1.0, prog / 0.7))
            alpha = 0.25 + 0.75 * min(1.0, prog / 0.7)
            mix = min(1.0, prog / 0.7)
        else:
            theta, alpha, mix = 1.0, 1.0, 1.0
        ...
        for t in range(max_loops):
            ...
            if self.training:
                ...
                p_hard_ste = hard + (p_soft - p_soft.detach())
                p = (1.0 - mix) * p_soft + mix * p_hard_ste
            else:
                p = F.one_hot(dl.argmax(dim=-1), 10).to(dl.dtype)
            if self.training:
