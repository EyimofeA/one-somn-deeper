...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.state_selector = nn.Linear(D_MODEL, NUM_PROTOTYPES)
        self.state_codebook = nn.Parameter(
            torch.empty(NUM_PROTOTYPES, D_MODEL)
        )
    ...
    def _quantize_state(self, candidate_state: Tensor) -> Tensor:
        soft_assignment = F.softmax(
            self.state_selector(candidate_state),
            dim=-1,
        )
        hard_index = soft_assignment.argmax(dim=-1)
    ...
    def forward(
        ...
        for column in range(MAX_COLUMNS):
            ...
            quantized_active = self._quantize_state(
                next_state.index_select(0, active_rows)
            )
            state = state.index_copy(
                0,
                active_rows,
                quantized_active.to(dtype=state.dtype),
            )
        ...
        for _ in range(FLUSH_STEPS):
            state = self._quantize_state(
                self.carry_cell(flush_input, state)
            )
