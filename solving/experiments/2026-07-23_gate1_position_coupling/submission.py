...
RoPE uses task-structured position coupling instead of physical token
positions. Digits of equal decimal significance share one coordinate across
numeric spans. Structural marker tokens retain distinct fixed coordinates.
...
def _coupled_position_ids(input_ids: Tensor, max_seq_len: int) -> Tensor:
    """Assign shared significance coordinates and distinct marker coordinates."""
    ...
    for position in range(input_ids.shape[1] - 1, -1, -1):
        ...
        positions[:, position] = torch.where(
            is_digit,
            significance + 1,
    ...
    for marker_token in range(1, DIGIT_TOKEN_START):
        positions = torch.where(
            input_ids == marker_token,
            torch.full_like(positions, max_seq_len + marker_token),
            positions,
        )
    return positions
...
def _rope_cos_sin(
    position_ids: Tensor, head_dim: int, dtype: torch.dtype
) -> tuple[Tensor, Tensor]:
    inv_freq = 1.0 / (
        ROPE_BASE
        ** (
...
class Model(nn.Module):
    ...
    def forward(
        ...
        position_ids = _coupled_position_ids(
            input_ids,
