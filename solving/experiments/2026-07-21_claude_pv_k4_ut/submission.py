"""Place-value + field-segment UT loop — claude code.

One-change ablation vs the e5 reference `depth_d32_k4_ut`.

Motivation (claude code): the task is digit arithmetic (square-and-reduce mod N).
The reference only gives the block absolute positions, so it must *learn* that
"position 4 is the units digit of N" from scratch under the 60s clock. We instead
inject two cheap, task-aligned structural signals derived from `input_ids` only
(no dataset access, no trapdoor):

  * field / segment id      — is this position inside the N span, X span, or T span?
  * place-value id          — how far is this digit from the units end of its span?
                              (Abacus-style least-significant-aligned index)

Everything else (d=32, 4 heads, shared Block x K=4, UT depth embeddings, AdamW
warmup+cosine, batch 256) is held identical to `depth_d32_k4_ut` so this is a
clean single change. Added state: ~2.2K elements (segment 4x32 + place 65x32).
"""
