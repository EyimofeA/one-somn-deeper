...
d_model=32, shared Pre-LN Block applied K=4 times. Each loop step adds a
learned depth embedding (Dehghani et al., 2018 — Universal Transformers) so
the shared block can distinguish timestep k. This is fixed-depth recurrence,
not Adaptive Computation Time (ACT).
"""
...
def _build_scheduler(
    ...
    # Wall-clock scheduler (validated fix, learnings/concepts/15-lr-schedules-wallclock.md).
    # The step-count `t_max = seconds * 8` heuristic assumes ~8 steps/s; this UT
    # loop measured ~48.7 steps/s on the L40S, so CosineAnnealingLR's un-clamped
    # periodicity past T_max produced a ~7600-step LR sawtooth instead of a
    # single anneal — see the 2026-07-22 t1only_probe_ut_k4 chart/NOTE.
    import time
