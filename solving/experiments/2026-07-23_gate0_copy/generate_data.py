#!/usr/bin/env python3
"""Generate the Gate 0 copy-X diagnostic in squaring_mod JSONL format."""
...
TEST_SIZE = 199
OOD_SIZE = 1000
DEFAULT_OUTPUT_DIR = Path("data/generated/gate0_copy_n10403_t1")
...
def make_record(x: int, split: str, index: int) -> dict[str, Any]:
    return {
        ...
        "generator_family": "gate0_copy_x",
        "input_ids": tokenize_prompt(x),
        "instance_id": f"gate0_copy_n{N}_t{T}_s{SEED}_{split}_{index:08d}",
        "label_exact": True,
        "label_method": "identity_copy_x",
        "labels": number_tokens(x),
        "modulus": N,
...
def self_test(records: list[dict[str, Any]]) -> None:
    ...
    for record in records:
        ...
        assert decode_digits(labels) == record["x"]
...
def dataset_config(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "label_format": "tail_aligned_decimal_copy_x",
        "label_method": "identity_copy_x",
