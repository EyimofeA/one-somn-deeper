"""One Layer Deeper — Hard submission (Fable architecture, our optimizer recipe).

Same architecture as fable_hard_h1 (design thesis unchanged — see that file's
docstring). The only change is build_optimizer: fable_hard_h1's original
WarmupSchedule held lr flat at 3e-4 forever after a 300-step warmup, no
decay. Loss was flat ~2.1-2.2 across both a 60s (e5) and 600s (m5) local run
— 10x more compute, zero movement, the signature of a stuck optimizer, not
an architecture that can't learn. This swaps in the AdamW recipe validated
across every other card in this project (lr=3e-3, wd=0.1, betas=(0.9,0.95))
plus the wall-clock warmup+cosine schedule from
learnings/concepts/15-lr-schedules-wallclock.md. One variable changed.
"""
...
class WallClockSchedule:
    ...
    Duck-typed (not a torch.optim.lr_scheduler.LRScheduler) so it works with
    any optimizer-shaped object, including a multi-optimizer wrapper —
    validate_optimizer only requires bundle.scheduler.step() to be callable.
    """
