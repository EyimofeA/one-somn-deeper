"""Middle-only depth loop — pre / shared-mid / post Transformer stack.

Adaptation of T²MLR (Cai et al., arXiv 2607.15178): middle recurrence often
beats full-stack recurrence. Here we map that idea to **depth** (fixed K=4
re-applications of one shared mid block), not temporal token cache across decode.

d_model=32, 4 heads. Depth embeddings (UT-style) are added only before each
mid-loop iteration, not on pre/post blocks.
"""
...
class Model(nn.Module):
    ...
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.depth_embedding = nn.Embedding(self.num_mid_loops, D_MODEL)
        self.block_pre = Block()
        self.block_mid = Block()
        self.block_post = Block()
    ...
    def forward(
        ...
        x = self.block_pre(x, attention_mask)
        for k in range(self.num_mid_loops):
            depth = self.depth_embedding(
                torch.tensor(k, device=input_ids.device, dtype=torch.long)
            )
            x = x + depth
            x = self.block_mid(x, attention_mask)
        x = self.block_post(x, attention_mask)
