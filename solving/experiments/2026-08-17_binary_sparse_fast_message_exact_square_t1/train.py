"""Local exact-square reducer with sparse parameter-free fast messages."""
...
class SparseMessageBinaryWorkState(base.BinaryWorkState):
    ...
    def forward(
        ...
        state = source.new_zeros(
            batch, self.channels, LANES, WORKSPACE_BITS
        )
        ...
        for step in range(self.updates):
            ...
            if step % 4 == 3:
                distance = distances[message_index % len(distances)]
                visible = visible + 0.25 * shifted_pair(visible, distance)
                message_index += 1
            state = self.cell(visible, dropout_mask)
        return self.readout(state[:, :, 2, :OPERAND_BITS]).squeeze(1)


base.BinaryWorkState = SparseMessageBinaryWorkState
