...
Input adds randomized Abacus digit-significance embeddings to the token
embedding while retaining RoPE. Numeric spans share significance indices:
ones digits receive beta, tens beta+1, and so on. Beta is one random
batch-wide offset in training and is fixed to 1 during evaluation.
...
def _abacus_indices(input_ids: Tensor, beta: Tensor) -> Tensor:
    """Index digits by right-to-left significance within each numeric span."""
    indices = torch.zeros_like(input_ids)
    significance = torch.zeros(
        input_ids.shape[0],
    ...
    for position in range(input_ids.shape[1] - 1, -1, -1):
        ...
        is_digit = (token >= DIGIT_TOKEN_START) & (token < DIGIT_TOKEN_STOP)
        indices[:, position] = torch.where(
            is_digit,
            beta + significance,
            torch.zeros_like(significance),
        )
        significance = torch.where(
            is_digit,
            significance + 1,
            torch.zeros_like(significance),
        )
...
class Model(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.abacus_embedding = nn.Embedding(
            ABACUS_K + spec.max_seq_len,
            D_MODEL,
            padding_idx=0,
        )
        ...
        nn.init.normal_(self.abacus_embedding.weight, std=0.02)
        with torch.no_grad():
            self.abacus_embedding.weight[0].zero_()
    ...
    def forward(
        ...
        if self.training:
            ...
            beta = torch.ones((), device=input_ids.device, dtype=torch.long)
        abacus_indices = _abacus_indices(input_ids, beta)
        x = self.token_embedding(input_ids) + self.abacus_embedding(abacus_indices)
