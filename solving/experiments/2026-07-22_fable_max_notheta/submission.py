...
class Model(nn.Module):
    ...
    def forward(
        ...
        # ABLATION (fable_max_notheta): isolates whether theta's continuous
        # anneal (1.0 -> 0.15 as progress -> 0.7) is what caused fable_max's
        # e5 collapse, vs. the discrete hard_st jump. theta is frozen at 1.0
        # (no sharpening at all, matching its value at progress=0); hard_st's
        # discrete prog>0.55 switch is UNCHANGED from the original.
        if self.training:
