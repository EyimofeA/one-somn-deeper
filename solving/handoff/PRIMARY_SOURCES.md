...
Compiled 2026-07-22; **rules/scoring refreshed 2026-08-03** against upstream `8a3c78d` (`Adding token_training_loss for sequence level loss`). Contains primary sources and raw logged metrics only.
Sources: [onelayerdeeper.ai](https://onelayerdeeper.ai), [onelayerdeeper.ai/problem](https://onelayerdeeper.ai/problem), [tilde-research/one-layer-deeper](https://github.com/tilde-research/one-layer-deeper) (local clone `competition/` @ `8a3c78d`), `one-layer leaderboard` CLI, `solving/experiments/metrics/*.jsonl`.

**Upstream delta since previous pin `79f0a09`:** (1) `TokenLossBatch` + mutually exclusive `token_training_loss` for sequence-aware losses (legacy flattened `training_loss` still valid); (2) Hard ranking adds next-rung accuracy tie-break (`seen_tiebreak_accuracy_percent` / `ood_n_tiebreak_accuracy_percent`) before submission time; (3) Rule 4 clarifies outer loop vs in-model recurrence/TRM/optimizer curvature; (4) Rule 14 now bans **data augmentation**; (5) README notes beta window Jul 31–Aug 2 10pm PT (closed) and Monday Aug 31 10pm PT deadline.
...
Verbatim copy of README headings `## Rules`, `### Submission contract`, and `### Compute tiers` (through the paragraph before `## CLI`), taken from upstream `8a3c78d`. Outer fence uses four backticks so nested Python fences stay intact.
...
Current Hard ranking (README Compute tiers, 2026-08-03 @ `8a3c78d`): Max T → OOD N Max T → exact accuracy at each profile's first uncertified rung → earlier submission time. Ladder T∈{1,2,4,8,16,32,64}. Easy/Medium still score mean exact accuracy; they also expose Max T / OOD N Max T as diagnostics.
```
...
> **Stale-dump warning (2026-08-03):** large verbatim dumps below predate `8a3c78d`. Prefer live files under `competition/` for `benchmark/api.py` (`TokenLossBatch`), `benchmark/runner.py`, `benchmark/validation.py` (mutual exclusion of loss callbacks), `data/squaring_mod.py`, and `service/competition.py`. Re-dump before trusting line-level citations.
