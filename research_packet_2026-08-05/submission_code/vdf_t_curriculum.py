"""Final-label-only VDF candidate with a time-staged, existing-row T curriculum."""
...
PAD, X_MARK, T_MARK, DIGIT_OFFSET = 0, 3, 4, 7
...
class SerialCell(nn.Module):
    ...
    def forward(self, value: Tensor, attention_mask: Tensor) -> Tensor:
        ...
        reversed_scans, _ = self.scan(reverse_valid(mixed, attention_mask))
        return mixed + reverse_valid(reversed_scans, attention_mask) * attention_mask[:, :, None]
...
class VDFModel(nn.Module):
    def __init__(self, spec: ModelSpec) -> None:
        ...
        self.norm, self.head = RMSNorm(WIDTH), nn.Linear(WIDTH, spec.vocab_size, bias=False)
    ...
    def forward(self, input_ids: Tensor, attention_mask: Tensor | None = None) -> tuple[Tensor, Tensor]:
        ...
        for depth in range(int(steps.max().item())):
            ...
            active_base, active_state = base.index_select(0, indices), state.index_select(0, indices)
            active_mask, active_register = mask.index_select(0, indices), register.index_select(0, indices)
            ...
            state = state.index_copy(0, indices, active_state.index_put((register_rows, register_columns), next_state))
            hidden = hidden.index_copy(0, indices, reduced)
        return self.head(self.norm(hidden)), steps


class Curriculum:
    """Training-only loss curriculum; it never changes a row's executed T."""
    def __init__(self) -> None:
        self.started, self.seconds, self.limit = 0.0, 1.0, 1

    def reset(self, seconds: float) -> None:
        self.started, self.seconds, self.limit = time.monotonic(), max(float(seconds), 1.0), 1

    def update(self) -> None:
        fraction = (time.monotonic() - self.started) / self.seconds
        self.limit = 1 if fraction < 1 / 3 else 2 if fraction < 2 / 3 else 3
...
def token_training_loss(batch: TokenLossBatch) -> Tensor:
    rows = batch.auxiliary.le(CURRICULUM.limit)[:, None]
    selected = batch.valid_mask & rows
    return F.cross_entropy(batch.logits[selected], batch.labels[selected])
...
SUBMISSION = Submission(build_model=build_model, build_optimizer=build_optimizer, token_training_loss=token_training_loss, batch_size=512, eval_batch_size=512)
